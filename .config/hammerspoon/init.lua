...
-- Setup this one hotkey first in case there are error in the config
hs.hotkey.bind(modHyper, 'y', hs.toggleConsole )

-- Reload config automatically
configFileWatcher = hs.pathwatcher.new(
  os.getenv("HOME") .. "/.hammerspoon/", function() hs.reload() end
)
...
-- Modal centre window
modal:bind('', 'c',
    cycleCalls(toGrid, {{0.1,0,0.8,1},{.04, 0, 0.92, 1},{0.22, 0.025, 0.56, 0.95}})
)

-- Size/position to one side of the screen
modal:bind('','left',
  cycleCalls(toGrid, {{0, 0, 0.5, 1}, {0, 0, 0.6, 1}, {0, 0, 0.4, 1}})
)
modal:bind('','right',
  cycleCalls(toGrid, {{0.5, 0, 0.5, 1}, {0.4, 0, 0.6, 1}, {0.6, 0, 0.4, 1}})
)
...
hs.hotkey.bind(modHyper, 'space',
  function() toggleMaximize(hs.window.focusedWindow()) end
)
hs.hotkey.bind(modHyper, 'f',
  function() hs.application.launchOrFocus( "Forklift" ) end
)
hs.hotkey.bind(modHyper, 'g',
  function() hs.application.launchOrFocus( "Finder" ) end
)
hs.hotkey.bind(modHyper, 'h', function() os.execute( "open ~" ) end )
hs.hotkey.bind(modHyper, 'q',
  function() hs.application.launchOrFocus( "Firefox" ) end
)
hs.hotkey.bind(modHyper, 'w',
  function() hs.application.launchOrFocus( "iTerm" ) end
)
-- hs.hotkey.bind(modHyper, 'm', toggleMicrophoneMute )
hs.hotkey.bind(modHyper, 'V',
